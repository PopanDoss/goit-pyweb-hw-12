...
def new_add_contact(contact: ContactAdd, user: User = Depends(get_current_user), db: Session = Depends(get_db)):
...
def read_contacts(user: User = Depends(get_current_user),  skip: int = 0, limit: int = 100, db: Session = Depends(get_db)):
...
def read_contact_id(contact_id: int, user: User = Depends(get_current_user), db: Session = Depends(get_db)):
...
def read_contact_search(user: User = Depends(get_current_user), firstname: str = None, lastname: str = None , email: str = None, db: Session = Depends(get_db)):
...
def update_contact(contact_id: int, contact: ContactUpdate,user: User = Depends(get_current_user), db: Session = Depends(get_db)):
...
def read_contacts_born_date(contacts_born_date: date, user: User = Depends(get_current_user), db: Session = Depends(get_db)):
...
def read_contacts_7days(user: User = Depends(get_current_user), db: Session = Depends(get_db)):
